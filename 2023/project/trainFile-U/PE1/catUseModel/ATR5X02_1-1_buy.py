...
catboost_model_path = '../163/catboost_model.bin'
...
def monitor_file():
    ...
    while True:
        ...
        minutes_rounded = minutes - (minutes % 5)

        if minutes == minutes_rounded and seconds == 0:
        # if 1:
            try:
                """执行操作"""
                data_input = getDataXAUUSD(360)
                # 删除列名为'time'的列
                ...
                label = model.predict(data_input[-1:])
                print(label)
                ...
                print(datetime.now(), label)

            except PermissionError:
                print('\n---> ---> error ...')
                time.sleep(60*5-10)  # 等待再次尝试尝试
