...
initialize_path = "../MT5 1-1/terminal64.exe"
symbol = "XAUUSDm"
...
def getDataXAUUSD(count):
    ...
    # 15分钟数据
    ratesM15 = getdata(symbol, mt5.TIMEFRAME_M15, 0, count)
    # 1小时数据
    ratesH1 = getdata(symbol, mt5.TIMEFRAME_H1, 0, count)
    # 4小时数据
    ratesH4 = getdata(symbol, mt5.TIMEFRAME_H4, 0, count)
    # 1天数据
    ratesD1 = getdata(symbol, mt5.TIMEFRAME_D1, 0, count)
    ...
    print('\n数据长度', len(ratesM5_frame), len(ratesM15_frame), len(ratesH1_frame), len(ratesH4_frame), len(ratesD1_frame))
    ...
    print("\n****计算最终shape\n", data_M5.shape, data_M15.shape, data_H1.shape, data_H4.shape, data_D1.shape)
    ...
    concat_df['time'] = pd.to_datetime(concat_df['time']).dt.strftime("%Y-%m-%d %H:%M:%S")
